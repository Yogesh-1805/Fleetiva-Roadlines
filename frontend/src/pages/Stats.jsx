...
// ✅ Role-aware amount picker
  const getAmount = (booking) => {
    if (role === "driver") return Number(booking.driverAmount) || 0;
    if (role === "admin") return Number(booking.platformAmount) || 0;
    return Number(booking.totalAmount) || 0; // customer
  };
  // useEffect(() => {
  ...
  // }, [role]);
  // In Stats.jsx useEffect
const getEndpoint = (role) => {
  if (role === "driver") return "/booking/driver/bookings";
  if (role === "admin") return "/booking/all"; // ← admin sees everything
...
useEffect(() => {
  ...
  //   };
  // }, [bookings]);
  // Replace your entire useMemo block with this:
...
//     arr.reduce((s, b) => s + (Number(b.amount) || 0) + (Number(b.gstAmount) || 0), 0);

//   const totalEarnings = calcAmount(paid);
//   const pendingAmount = calcAmount(pending);
...
//     })
//     .reduce((s, b) => s + (Number(b.amount) || 0) + (Number(b.gstAmount) || 0), 0);

//   const pastContracts = list.filter((b) => b.status === "delivered");
//   const currentWork = list.filter((b) => b.status === "assigned" || b.status === "in-transit");

//   const avgPerDelivery = pastContracts.length > 0 ? calcAmount(pastContracts) / pastContracts.length : 0;
//   const futureEstimate = currentWork.length > 0 ? calcAmount(currentWork) : avgPerDelivery;

//   return { totalEarnings, pendingAmount, paidCount: paid.length, pendingCount: pending.length, pastContracts, currentWork, futureEstimate, thisMonthEarnings };
// }, [bookings]);
...
const {
    ...
    // ✅ Uses getAmount — role-aware
    const totalEarnings = paid.reduce((s, b) => s + getAmount(b), 0);
    const pendingAmount = pending.reduce((s, b) => s + getAmount(b), 0);
    ...
    const avgPerDelivery =
      pastContracts.length > 0
        ? pastContracts.reduce((s, b) => s + getAmount(b), 0) /
          pastContracts.length
        : 0;

    const futureEstimate =
      currentWork.length > 0
        ? currentWork.reduce((s, b) => s + getAmount(b), 0)
        : avgPerDelivery;
  ...
  }, [bookings, role]); // ✅ role is a dependency
      ...
      <div className="stats-content">
        ...
        <section className="stats-section">
          ...
          <div className="stats-earnings-grid">
            ...
            <div className="stats-earnings-card stats-earnings-total">
  <span className="stats-earnings-icon">💰</span>
  <div className="stats-earnings-body">
    <span className="stats-earnings-label">
      {role === "customer" ? "Total spent" : role === "driver" ? "Total earned" : "Total revenue"}
    </span>
    <span className="stats-earnings-value">{formatMoney(totalEarnings)}</span>
    <span className="stats-earnings-meta">
      {paidCount} paid deliver{paidCount !== 1 ? "ies" : "y"}
    </span>
  </div>
  ...
  <div className="stats-earnings-body">
    <span className="stats-earnings-label">
      {role === "customer" ? "Amount due" : role === "driver" ? "Pending payout" : "Pending payments"}
    </span>
    <span className="stats-earnings-value">{formatMoney(pendingAmount)}</span>
    <span className="stats-earnings-meta">
      {pendingCount} booking{pendingCount !== 1 ? "s" : ""}
    </span>
  </div>
