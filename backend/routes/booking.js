...
const driverCutPercent = Number(process.env.DRIVER_CUT_PERCENT) || 0.12; // 12% to driver
const driverAmount = Math.round(total * driverCutPercent);
const platformAmount = total - driverAmount;
  ...
  const booking = await Booking.create({
    ...
    totalAmount: total,       // 10000 → customer sees this
    driverAmount,             // 1200  → driver sees this
    platformAmount,           // 8800  → admin sees this
  });
...
router.get('/customer/bookings', authenticate, authorize('customer','admin'), asyncHandler(async (req, res) => {
  // const bookings = await Booking.find({ customer: req.user.userId })
  // const query = req.user.role === 'admin' ? {} : { customer: req.user.userId };
  // const bookings = await Booking.find(query)
  //   .populate('load')
  //   .sort({ createdAt: -1 });
  // res.json(bookings);
  const query = req.user.role === 'admin' ? {} : { customer: req.user.userId };
  const bookings = await Booking.find(query).populate('load').sort({ createdAt: -1 });
  res.json(bookings);
}));

router.get('/driver/bookings', authenticate, authorize('driver','admin'), asyncHandler(async (req, res) => {
  // const bookings = await Booking.find({ driver: req.user.userId })
  // const query = req.user.role === 'admin' ? {} : { driver: req.user.userId };
  // const bookings = await Booking.find(query)
  //   .populate('load')
  //   .sort({ createdAt: -1 });
  // res.json(bookings);
